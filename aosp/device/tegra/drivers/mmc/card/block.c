...
static struct io_monitor_data io_ringbuf[MAX_RINGBUF_SIZE];
EXPORT_SYMBOL(io_ringbuf);
			       ...
			       struct mmc_queue *mq)
{
	...
	printk("%s: address:%x blocks:%x\n",
           ((brq->cmd.opcode == readcmd)? "Read":"Write"),
           brq->cmd.arg, brq->data.blocks);
     
    if(ringbuf_idx>(MAX_RINGBUF_SIZE-1)){
		ringbuf_idx = 0;
	}
	io_ringbuf[ringbuf_idx] = (struct io_monitor_data){ .opcode=(brq->cmd.opcode), .pid=(current->pid), .address=(brq->cmd.arg), .blocks=(brq->data.blocks) };
	ringbuf_idx++;
